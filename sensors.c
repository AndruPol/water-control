...
static uint16_t samples[SENSORSALL];			// описание датчиков
static binary_semaphore_t adcsem, adc_cbsem;	// семафоры чтения ADC

float mcu_temp;

#define ADCRead_PRIO			(NORMALPRIO)
...
#if MCUTEMP
		    mcu_temp = TEMPCALC(samples[TEMPINT], samples[VREFINT]);
#endif
		}
		chBSemSignal(&adcsem);
	}
